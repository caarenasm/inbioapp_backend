<div class="m-3 p-2 bg-gray-50 shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
    ...
    <div class="grid grid-cols-2">
        <div class="m-3">
            ...
            <input type="text" name="title" id="title" class="w-full rounded-xl text-gray-500 border-gray-300" value="{{old('title', $post->title)}}">
            @error('title')
        ...
        <div class="m-3">
            ...
            <select type="text" name="author" id="author" class="w-full rounded-xl text-gray-500 border-gray-300">
        ...
        <div class="m-3">
            ...
            <input type="date" name="start_date" id="start_date" class="w-full rounded-xl text-gray-500 border-gray-300" value="{{old('start_date', $post->start_date)}}">
            @error('start_date')
            <small class="text-red-500">* {{$message}}</small>
            @enderror
        </div>
        ...
        <div class="m-3">
            ...
            <input type="date" name="end_date" id="end_date" class="w-full rounded-xl text-gray-500 border-gray-300" value="{{old('end_date', $post->end_date)}}">
            @error('end_date')
            <small class="text-red-500">* {{$message}}</small>
            @enderror
        </div>
        ...
        <div class="m-3">
            ...
            <label class="block"><input type="checkbox" name="categoria[]" value="{{$category->id}}" {{ (is_array(old('categoria', $post->categorias()->pluck('categorias_blog_id')->all())) && in_array($category->id, old('categoria', $post->categorias()->pluck('categorias_blog_id')->all()))) ? ' checked' : '' }}> {{$category->name}}
            </label>
            @endforeach
            @error('categoria')
            <small class="text-red-500">* {{$message}}</small>
        ...
        <div class="m-3">
            ...
            <input type="text" name="slug" id="slug" class="w-full rounded-xl text-gray-500 border-gray-300" value="{{old('slug', $post->slug)}}" readonly>
            @error('slug')
            <small class="text-red-500">* {{$message}}</small>
            @enderror
        </div>
        <div class="m-3">
            ...
            <input type="text" name="seo_title" id="seo_title" class="w-full rounded-xl text-gray-500 border-gray-300" value="{{old('seo_title', $post->seo_title)}}">
            @error('seo_title')
            <small class="text-red-500">* {{$message}}</small>
            @enderror
        </div>
        ...
        <div class="m-3">
            ...
            <textarea name="seo_description" id="seo_description" class="w-full rounded-xl text-gray-500 border-gray-300">{{old('seo_description', $post->seo_description)}}</textarea>
        ...
        <div class="w-1/2 m-3">
            ...
            <select wire:model="selectedState" class="form-control rounded-xl border-gray-300" id="resolucion" name="resolucion" required>
                ...
                <option value="{{ $resolucion->id }}" @if (old('resolucion')==$resolucion->id || $resolucion->id == $post->resolucion) selected @endif> {{ $resolucion->resolucion }}
                </option>
                @endforeach
            </select>
        ...
        <div class="m-3">
            ...
            <small class="text-red-500">* {{$message}}</small>
            @enderror
            <textarea name="text" id="text" class="w-full rounded-xl text-gray-500 border-gray-300">{{old('text', $post->content)}}</textarea>
        ...
        <div class="flex space-x-2">
            ...
            <div class="flex-grow m-3">
                <label><input type="radio" name="published" value="0" required {{old('published', $post->published) == '0' ? 'checked' : '' }}> Borrador</label>
