<x-app-layout>
    <div class="p-2 bg-white">
        ...
        <div class="m-3  w-1/3 p-3 bg-gray-50 shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    ...
                    <div class="w-auto mb-3">
                        ...
                        <select class="form-control rounded-xl border-gray-300" id="alimento_id" name="alimento_id"
                            ...
                            @foreach ($alimentos_enfermedad as $alimento)
                                <option value="{{ old('alimento_id') == $alimento->alimento_id }}"
                                    @if (old('alimento_id') == $alimento->alimento_id || $alimento->alimento_id == $enfermedad_alimento->alimento_id) selected @endif>{{ $alimento->nombre }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="w-auto mb-3">
                        ...
                        <select class="form-control rounded-xl border-gray-300" id="recomendacion" name="recomendacion">
                            ...
                            @foreach ($estados as $estado)
                                <option value="{{ $estado->id }}" @if (old('recomendacion') == $estado->id || $estado->id == $enfermedad_alimento->recomendacion) selected @endif>
                                    {{ $estado->estado_semaforo }}</option>
                            @endforeach
                        </select>
                        @error('recomendacion')
                            <small class="text-red-500">* {{ $message }}</small>
