<x-app-layout>
    ...
    <div class="p-2 bg-white">
        <h2 class="text-2xl py-2 text-fondo-verde font-extrabold">Países para envíos</h2>

        <div class="grid grid-cols-2 space-x-5">

            <div>
                @if($pais->id)
                    <h3 class="text-xl py-2 text-fondo-verde font-extrabold">Modificar país</h3>
                    <form method="post" action="{{route('envio-pais.update', $pais)}}">
                        @csrf
                        @method('put')
                        @include('livewire/admin/envios/paises-form')

                        <x-forms.button type="submit" text="Guardar modificación"/>
                    </form>
                @else
                    <h3 class="text-xl py-2 text-fondo-verde font-extrabold">Nuevo país</h3>
                    <form method="post" action="{{route('envio-pais.store')}}">
                        @csrf
                        @include('livewire/admin/envios/paises-form')

                        <x-forms.button type="submit" text="Agregar país"/>
                    </form>
                @endif
            ...
            <div>
                <h3 class="text-xl py-2 text-fondo-verde font-extrabold">Países existentes</h3>
                @include('livewire/admin/envios/paises-tabla')
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        ¿Estás totalmente seguro/a de eliminar este país? Esta acción no se puede
                                        deshacer.
                                    </p>
                                </div>
                            </div>
